    ...
    if packet.haslayer(DNS):
        ...
        if packet[DNS].qr == 0:
           ...
           # packet[DNS].show()
            ...
            print ("NAME_REF length", len(name_ref))
            pprint.pprint(name_ref)
...
#                del Header[-1]
            ...
            print ("R,", len(bytes(packet[DNS])), ',', len (cbor.dumps(Header)), ",", 
                       (len(bytes(packet[DNS]))- len (cbor.dumps(Header) )), ',', 
                   ((len(bytes(packet[DNS]))- len (cbor.dumps(Header) ))/len(bytes(packet[DNS])))*100,
                    ", ==, ", len(name_ref), packet[DNSQR].qtype)
            
             #print(bytes(packet[DNS]))
             #print("=========================")
             #print(cbor.dumps(Header))
            ...
            #RESULTS = [str(wire), str(ecborr),  str(p)]
            RESULTS = [str(p), str(lname)]

            with open("abc4.csv",'a', newline = '', encoding = 'utf-8') as f:
                writer = csv.writer(f,  dialect='excel-tab',  lineterminator = '\n')
                writer.writerow(RESULTS)
                f.close()
