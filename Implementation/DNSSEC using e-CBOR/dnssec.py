...
def insert_rr_names(rr, nb_elm):
    for i in range(nb_elm):
        ...
        elif rr[i].type == 43: # DS
            insert_name(rr[i].rrname.decode())
            

        elif rr[i].type == 46: # RRSIG
            insert_name(rr[i].rrname.decode())
            insert_name(rr[i].signersname.decode()) 

        elif rr[i].type == 47: # NSEC
            insert_name(rr[i].rrname.decode())
            insert_name(rr[i].nextname.decode())
            
        elif rr[i].type == 48: # DNSKEY
            insert_name(rr[i].rrname.decode())
...
def create_rr(rr, nb_elm):
    ...
    for i in range(nb_elm):
        if rr[i].type == 1:  #A 
            ...
            ref_index[rr[i].signersname.decode()],rr[i].ttl]
            ...
            """ [type, name(ref), DS(ref), ttl,... (class), (rr[i].digest)] """
            c_rr = [43, ref_index[rr[i].rrname.decode()], rr[i].ttl, rr[i].keytag, 
            rr[i].algorithm, rr[i].digesttype, rr[i].digest ]


        elif rr[i].type == 46: # RRSIG
            """ [type, name(ref), RRSIG(ref), ttl,... (class), (rr[i].signature)] """
            c_rr = [46, ref_index[rr[i].rrname.decode()],ref_index[rr[i].signersname.decode()], 
            rr[i].originalttl, rr[i].labels, rr[i].typecovered, rr[i].algorithm, rr[i].keytag, 
            rr[i].expiration, rr[i].inception, rr[i].signature]


        elif rr[i].type == 47: # NSEC
            """ [type, name(ref), NSEC(ref), ttl,...(class)] """
            c_rr = [47, ref_index[rr[i].rrname.decode()],ref_index[rr[i].nextname.decode()], 
            rr[i].ttl]
        
        elif rr[i].type == 48: # DNSKEY 
            """ [type, name(ref), DNSKEY(ref), ttl,... (class), (rr[i].publickey)] """
            c_rr = [48, ref_index[rr[i].rrname.decode()], rr[i].ttl,rr[i].protocol, 
            rr[i].algorithm, rr[i].publickey]

        elif rr[i].type == 41: # OPT
            """ [type, name(ref), ttl,...(class)] """
            c_rr = [41, ref_index[rr[i].rrname.decode()], rr[i].z, rr[i].rclass, rr[i].rdata]
...
dns_packets = rdpcap('dnssecokk.pcap')
for packet in dns_packets:
    if packet.haslayer(DNS):
        ...
        if packet[DNS].qr == 0:
            ...
            #             ((len(bytes(packet[DNS]))- len (cbor.dumps(Header) ))/len(bytes(packet[DNS])))*100,
            #         ", ==, ", packet[DNSQR].qtype)

            #print(hexdump(bytes(packet[DNS])))
            #print("=========================")
            #print(hexdump(cbor.dumps(Header)))


            p= ((len(bytes(packet[DNS]))- len (cbor.dumps(Header) ))/len(bytes(packet[DNS])))*100
             # p2 = (p1/len(bytes(packet[DNS])))*100

            wire = len(bytes(packet[DNS]))
            ecborr = len (cbor.dumps(Header))

            RESULTS = [str(wire), str(ecborr),  str(p)]

            # with open("respSOA.csv",'a', newline = '', encoding = 'utf-8') as f:
...
#                del Header[-1]
            ...
            dns_natif= bytes(packet[DNS])
            dns_cbor = cbor.dumps(Header)

               #dns_natif.pdfdump()


            #dns_natif.pdfdump(('dns_natif.pdf'))
            #dns_cbor.pdfdump('ddd.pdf')
            #print(dns_cbor)
